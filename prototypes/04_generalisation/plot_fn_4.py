...
from sweep_fn_4 import extract_resource_trips, hex_dist
...
def get_run_env(run):
    ...
    radius = int(env_kwargs.get("radius", 1))

    # prefer the real lists stored on the run
    water_coords = run.get("water_coords", None)
    food_coords = run.get("food_coords", None)

    # fall back to single-coord 03b runs
    if water_coords is None:
        wc = env_kwargs.get("water_coord", (-radius, 0))
        water_coords = [wc]
    if food_coords is None:
        fc = env_kwargs.get("food_coord", (0, radius))
        food_coords = [fc]

    return radius, [tuple(c) for c in water_coords], [tuple(c) for c in food_coords]
...
# -----------------------------
def _comfort_weights(run, over_w=None, under_w=None):
    ak = run.get("agent_kwargs", {})
    ow = ak.get("over_w", over_w if over_w is not None else 0.3)
    uw = ak.get("under_w", under_w if under_w is not None else 1.0)
    return float(ow), float(uw)
...
def plot_phase_heatmap(
    ...
    ideal_h=1.0,
    ideal_s=1.0,
    over_w=None,      # None -> read from run's agent_kwargs
    under_w=None,     # None -> read from run's agent_kwargs
    k=3.0,
):
    h = np.asarray(run["hydration_eval"])
    s = np.asarray(run["satiation_eval"])

    ow, uw = _comfort_weights(run, over_w, under_w)
    ...
    d2 = (uw * h_under**2 + ow * h_over**2
          + uw * s_under**2 + ow * s_over**2)
    Z = 2 * np.exp(-k * d2) - 1
    Z = np.clip(Z, -1, 1)
...
#region COMFORT vs REWARD

def plot_comfort_vs_reward(names=None, res_obj=None, save=False, death_cap=300):
    """Each seed = one point at (mean_comfort, mean_reward). The y=x line is
    'no deaths' (reward == comfort); the vertical drop below it is the death
    tax that comfort can't see. Colour = eval deaths, marker = model."""
    if res_obj is None: res_obj = res
    if names   is None: names   = list(res_obj.keys())

    comfort, reward, deaths, is_double = [], [], [], []
    for name in names:
        for run in res_obj[name]["ranked"]:            # every seed, not a representative
            comfort.append(run.get("mean_comfort", np.nan))
            reward.append(run.get("mean_reward",  np.nan))
            deaths.append(run.get("death_count_eval", np.nan))
            is_double.append("double" in name)
    comfort=np.asarray(comfort,float); reward=np.asarray(reward,float)
    deaths=np.asarray(deaths,float);   is_double=np.asarray(is_double)

    fig, ax = plt.subplots(figsize=(8.5, 8))
    lo = min(np.nanmin(comfort), np.nanmin(reward)) - 0.08
    hi = max(np.nanmax(comfort), np.nanmax(reward)) + 0.08

    ax.fill_between([0, hi], lo, 0, color="crimson", alpha=0.05, zorder=0)   # comfort>0, reward<0 = farm zone
    ax.plot([lo, hi], [lo, hi], "--", color="0.45", lw=1.3, zorder=1, label="reward = comfort (no deaths)")

    norm = mcolors.Normalize(0, death_cap); sc = None
    for mask, mk, lbl in [(~is_double, "o", "vanilla"), (is_double, "^", "double")]:
        if mask.any():
            sc = ax.scatter(comfort[mask], reward[mask], c=np.clip(deaths[mask], 0, death_cap),
                            cmap="inferno_r", norm=norm, marker=mk, s=110,
                            edgecolor="black", linewidth=0.5, zorder=3, label=lbl)
    cb = fig.colorbar(sc, ax=ax, shrink=0.85, pad=0.02); cb.set_label(f"eval deaths (clipped @ {death_cap})")

    ax.axhline(0, color="0.85", lw=0.7, zorder=0); ax.axvline(0, color="0.85", lw=0.7, zorder=0)
    ax.set_xlim(lo, hi); ax.set_ylim(lo, hi); ax.set_aspect("equal")
    ax.set_xlabel("mean comfort")
    ax.set_ylabel("mean reward  (carries the death penalty)")
    ax.set_title("comfort vs reward — gap below the diagonal is the death tax", fontsize=10)
    ax.legend(fontsize=8, loc="lower right")
    plt.tight_layout()
    if save: _save_fig(fig, "comfort_vs_reward")
    plt.show()

from matplotlib.lines import Line2D

def _learn_every(run, default=20):
    for d in (run.get("agent_kwargs", {}), run.get("sim_kwargs", {})):
        if "learn_every" in d:
            return int(d["learn_every"])
    return default


def first_food_tick(run, train_only=True):
    _, _, food_coord = get_run_env(run)
    coords = np.asarray(run["coordinates_T"], dtype=int)
    if train_only:
        coords = coords[:eb_of(run)]
    mask = (coords[:, 0] == food_coord[0]) & (coords[:, 1] == food_coord[1])
    hits = np.flatnonzero(mask)
    return int(hits[0]) if len(hits) else None


def _sigma_solved(run, comfort_thr=0.7, food_frac=0.01):
    _, _, food_coord = get_run_env(run)
    ce = eval_slice(run, "coordinates_T").astype(int)
    if len(ce) == 0:
        return False
    food_frac_eval = ((ce[:, 0] == food_coord[0]) & (ce[:, 1] == food_coord[1])).mean()
    return (food_frac_eval >= food_frac) and (run.get("mean_comfort", -1) >= comfort_thr)


def plot_sigma_decay_compare(names=None, res_obj=None, save=False):
    if res_obj is None:
        res_obj = res
    if names is None:
        names = list(res_obj.keys())

    names = [nm for nm in names
             if any(len(r.get("sigma_w_mean", [])) > 0 for r in res_obj[nm]["ranked"])]
    if not names:
        print("no sigma_w_mean logged in selected configs (noisy_DQN only)")
        return
    ...
    for ax, nm in zip(axes, names):
        runs = res_obj[nm]["ranked"]
        n_solved = 0
        for run in runs:
            sig = run.get("sigma_w_mean", [])
            if len(sig) == 0:
                continue
            sig = np.asarray([float(v) for v in sig], dtype=float)
            le = _learn_every(run)
            n_tr = max(1, eb_of(run) // le)
            sig = sig[:n_tr]
            x = np.arange(len(sig)) * le

            solved = _sigma_solved(run)
            n_solved += solved
            color = "tab:green" if solved else "tab:red"
            ax.plot(x, sig, color=color, lw=0.9, alpha=0.55,
                    zorder=3 if solved else 2)

            ff = first_food_tick(run)
            if ff is not None:
                idx = min(len(sig) - 1, ff // le)
                ax.scatter([ff], [sig[idx]], color=color, edgecolor="black",
                           s=60, zorder=5)

        ax.set_title(f"{nm}   ({n_solved}/{len(runs)} solved)", fontsize=10)
        ax.set_xlabel("training tick")
        ax.grid(alpha=0.15)
    axes[0].set_ylabel("mean |σ_w|")

    proxies = [
        Line2D([0], [0], color="tab:green", lw=2, label="solved"),
        Line2D([0], [0], color="tab:red", lw=2, label="camper"),
        Line2D([0], [0], marker="o", color="w", markerfacecolor="gray",
               markeredgecolor="black", markersize=8, label="σ at first food contact"),
    ]
    axes[-1].legend(handles=proxies, fontsize=8, loc="upper right")
    fig.suptitle("σ_w decay vs first food contact", fontsize=12)
    plt.tight_layout()
    if save:
        _save_fig(fig, "cmp_sigma_decay_" + "_vs_".join(names))
    plt.show()

#plot_comfort_vs_reward()              # all 8 configs
...
def _draw_hex_panel(ax, cw, all_coords, xs, ys, norm, cmap, water_coords, food_coords, title):
    lookup = {}
    if len(cw) > 0:
    ...
    _draw_resource_outlines(ax, water_coords, food_coords)
...
def plot_hex_eval_compare(runs, eval_window=None, save=False):
    ...
    for nm, run in runs:
        ...
        radius, water_coords, food_coords = get_run_env(run)
        ...
        panels.append((nm, ev, all_coords, xs, ys, water_coords, food_coords))
    ...
    fig.suptitle("eval hex occupancy — " + " vs ".join(nm for nm, _ in runs) +
                 "\nblue = water, orange = food", fontsize=13)
    if save:
...
def plot_death_gaps_eval_compare(runs, shared_bins=True, save=False):
    ...
    plt.show()
def _draw_resource_outlines(ax, water_coords, food_coords):
    for wc in water_coords:
        xw, yw = axial_to_xy(*wc)
        ax.add_patch(Polygon(hex_vertices(xw, yw, scale=1.02), closed=True,
                             facecolor="none", edgecolor="deepskyblue", linewidth=2.5))
    for fc in food_coords:
        xf, yf = axial_to_xy(*fc)
        ax.add_patch(Polygon(hex_vertices(xf, yf, scale=1.02), closed=True,
                             facecolor="none", edgecolor="darkorange", linewidth=2.5))


def plot_hex_occupancy_log(run, name="run", train_window=10_000, eval_window=None, save=False):
    coords_T = np.asarray(run["coordinates_T"], dtype=int)
    eb = eb_of(run)
    radius, water_coords, food_coords = get_run_env(run)
    ...
    sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm); sm.set_array([])
    fig.colorbar(sm, cax=cax).set_label("ticks in window (log)")
    fig.suptitle(f"hex occupancy over training — {name}\nblue = water, orange = food", fontsize=14)
    if save:
...
def plot_trip_lengths(run, name="run", max_trip_ticks=300, save=False):
    ...
    success_shortest = np.array([tr["shortest"] for tr in success], dtype=float)

    fig, ax = plt.subplots(figsize=(8, 4.8))
    if len(success_moves) == 0:
        ax.text(0.5, 0.5, "no successful water → food trips",
                ha="center", va="center", transform=ax.transAxes)
    else:
        bins = np.arange(success_moves.min(), success_moves.max() + 2) - 0.5
        ax.hist(success_moves, bins=bins, edgecolor="black")
        ax.axvline(np.median(success_shortest), ls="--", c="black",
                   label=f"median shortest = {np.median(success_shortest):.0f}")
        ax.axvline(np.median(success_moves), ls=":", c="gray",
                   label=f"median moves = {np.median(success_moves):.1f}")
        ax.legend(fontsize=8)

    ax.set_title(f"successful water → food trip lengths\n{run_label(run, name)}\n"
                 f"trips={len(trips)}, success={len(success)}", fontsize=9)
...
__all__ = [n for n, v in list(globals().items()) 
    if not n.startswith("__") and getattr(v, "__module__", None) == __name__]