...
from sweep_fn_v5 import extract_resource_trips, hex_dist
...
def first_food_tick(run, train_only=True):
    _, _, food_coords = get_run_env(run)
    food_set = {tuple(c) for c in food_coords}
    coords = np.asarray(run["coordinates_T"], dtype=int)
    if train_only:
        coords = coords[:eb_of(run)]
    mask = np.fromiter((tuple(c) in food_set for c in coords), bool, len(coords))
    hits = np.flatnonzero(mask)
    return int(hits[0]) if len(hits) else None


def _sigma_solved(run, route_thr=0.0, eat_thr=0.0):
    m = run.get("metrics", {})
    return (m.get("two_way_route_success_min", 0) > route_thr
            and m.get("eat_rate_at_food", 0) > eat_thr)
