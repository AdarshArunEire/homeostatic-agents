...
def compute_eval_metrics(run, env_kwargs):
    ...
    # -----------------------------
    # segment-aware resource + route metrics
    # -----------------------------
    eval_segments = run.get("eval_segments", None)
    eb = eval_boundary(run)

    if eval_segments:
        # build per-segment slices of the eval arrays, each against its own map
        # segment t_start is GLOBAL; convert to eval-local index
        seg_bounds = []
        for i, seg in enumerate(eval_segments):
            start_local = int(seg["t_start"]) - eb
            end_local = (int(eval_segments[i + 1]["t_start"]) - eb
                         if i + 1 < len(eval_segments) else len(coords_eval))
            start_local = max(0, start_local)
            end_local = min(len(coords_eval), end_local)
            if end_local > start_local:
                seg_bounds.append((seg, start_local, end_local))

        # pooled accumulators
        at_water_total = at_food_total = 0
        eat_on_food = food_ticks = 0
        drink_on_water = water_ticks = 0
        full_eat_on_food = 0
        all_wf_trips, all_fw_trips = [], []

        for seg, a, z in seg_bounds:
            wset = set(map(tuple, seg["water_coords"]))
            fset = set(map(tuple, seg["food_coords"]))
            seg_coords = coords_eval[a:z]
            seg_actions = actions_eval[a:z]
            seg_death = death_eval[a:z]

            aw = np.array([tuple(c) in wset for c in seg_coords], dtype=bool)
            af = np.array([tuple(c) in fset for c in seg_coords], dtype=bool)

            at_water_total += int(aw.sum())
            at_food_total += int(af.sum())

            if af.any():
                eat_on_food += int(np.isin(seg_actions[af], EAT_IDS).sum())
                full_eat_on_food += int((seg_actions[af] == FULL_EAT_ID).sum())
                food_ticks += int(af.sum())
            if aw.any():
                drink_on_water += int(np.isin(seg_actions[aw], DRINK_IDS).sum())
                water_ticks += int(aw.sum())

            # trips scored against THIS segment's own resources
            all_wf_trips += extract_resource_trips(
                coords_eval=seg_coords, death_eval=seg_death,
                source_coords=list(wset), target_coords=list(fset))
            all_fw_trips += extract_resource_trips(
                coords_eval=seg_coords, death_eval=seg_death,
                source_coords=list(fset), target_coords=list(wset))

        n = len(coords_eval)
        metrics["water_visit_pct"] = float(100 * at_water_total / n)
        metrics["food_visit_pct"] = float(100 * at_food_total / n)
        metrics["eat_rate_at_food"] = safe_ratio(eat_on_food, food_ticks)
        metrics["full_eat_rate_at_food"] = safe_ratio(full_eat_on_food, food_ticks)
        metrics["drink_rate_at_water"] = safe_ratio(drink_on_water, water_ticks)

        metrics.update(summarise_trips(all_wf_trips, "water_to_food"))
        metrics.update(summarise_trips(all_fw_trips, "food_to_water"))
        ...
        metrics["n_eval_segments"] = len(seg_bounds)
...
def slim_run(raw_run, config, seed, run_time=None, train_fn=None):
    ...
    for k in ("eval_segments", "ticks_at_water_eval", "ticks_at_food_eval",
              "n_timeouts_eval", "n_train_maps", "n_eval_maps"):
        if k in raw_run:
            slim[k] = raw_run[k]
